...
module.exports = {
	...
	run(message) {
		...
		let command = NMDClient.commands.get(cmd)
		if (!command) command = NMDClient.commands.get(NMDClient.aliases.get(cmd));
		...
		if (command) {
			if (command.cooldown) {
				...
				if (command.userPerms || command.botPerms) {
					...
					if (!message.guild.members.cache.get(NMDClient.user.id).permissions.has(PermissionsBitField.resolve(command.botPerms || []))) {
				...
				command.run(NMDClient, message, args)
